...
class PG:
    ...
    def __init__(self, config):
        self.config = config
        self.tracker = "PG"
        self.source_flag = "PeerGarden"
        self.upload_url = "https://peergarden.org/api/torrents/upload"
        self.search_url = "https://peergarden.org/api/torrents/filter"
        self.torrent_url = "https://peergarden.org/torrents/"
            ...
            try:
                ...
                if "data" in resp_json and resp_json["data"]:
                    await common.add_tracker_torrent(
                        ...
                        "https://peergarden.org/torrents/" + str(resp_json["data"]),
                        headers=headers,
                        params=params,
    ...
    async def edit_name(self, meta):
        ...
        if name_type == "DVDRIP":
            if meta.get("category") == "MOVIE":
                ...
                pg_name = pg_name.replace(f"{meta['source']}", f"{resolution}", 1)
                pg_name = pg_name.replace(
                    f"{meta['video_codec']}",
