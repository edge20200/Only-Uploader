...
class ZNTH:
    ...
    def __init__(self, config):
        self.config = config
        self.tracker = "ZNTH"
        self.source_flag = "znth.cx"
        self.upload_url = "https://znth.cx/api/torrents/upload"
        self.search_url = "https://znth.cx/api/torrents/filter"
        self.torrent_url = "https://znth.cx/torrents/"
            ...
            try:
                ...
                if "data" in resp_json and resp_json["data"]:
                    await common.add_tracker_torrent(
                        ...
                        "https://znth.cx/torrents/" + str(resp_json["data"]),
                        headers=headers,
                        params=params,
    ...
    async def edit_name(self, meta):
        ...
        if name_type == "DVDRIP":
            if meta.get("category") == "MOVIE":
                ...
                znth_name = znth_name.replace(f"{meta['source']}", f"{resolution}", 1)
                znth_name = znth_name.replace(
                    f"{meta['video_codec']}",
